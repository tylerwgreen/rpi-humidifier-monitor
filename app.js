var app = {
	init: function(){
		...
		app.logger = logger.getLogger('app', 'silly'); // set log level for the app
		...
		app.htu21dSensor = require('htu21dSensor');
		app.htu21dSensor.init({
			logger: logger,
			config: app.config.get('htu21dSensor'),
		})
			.then(function(){
			...
			.catch(function(err){
				app.logger.error(['init()', 'Error initializing htu21dSensor', err]);
			});
	},
	readings: {
		init: function(){
		...
		update: function(){
			...
			app.htu21dSensor.updateReading()
				.then(function(reading){
					app.logger.silly(['readings.update()', 'htu21dSensor.updateReading() success', reading]);
					app.readings.handleReading();
				})
				.catch(function(err){
					app.logger.error(['readings.update()', 'htu21dSensor.updateReading() error', err]);
		...
		handleReading: function(){
			...
			app.htu21dSensor.validateHumidityReading()
				.then(function(data){
					app.logger.silly(['readings.handleReading()', 'htu21dSensor.validateHumidityReading() success', JSON.stringify(data)]);
					...
					app.logger.silly(['readings.handleReading()', 'htu21dSensor.validateHumidityReading() error', JSON.stringify(data)]);
		...
		logReading: function(){
			...
			app.dataSheet.addRow({
				...
				temperature: app.htu21dSensor.getTemperatureReading(),
				humidity: app.htu21dSensor.getHumidityReading(),
